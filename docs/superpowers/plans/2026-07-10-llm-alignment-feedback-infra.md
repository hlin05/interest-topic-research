# LLM Alignment & Feedback Infrastructure Topic Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add `llm-alignment-feedback-infra` as a new weekly-researched topic covering the specification-to-training pipeline for LLM alignment (rubrics, contracts, RLAIF, LLM-as-judge, scalable oversight).

**Architecture:** Two files need to be created or modified. `topics.yml` gets a new topic entry which the existing `scripts/topic_config.py` reads dynamically — no workflow changes needed. `topics/llm-alignment-feedback-infra/README.md` is the human-facing resource list, seeded with initial sections and a few landmark resources.
...
**Files:**
- Modify: `topics.yml` (append after the `rlhf-agentic` entry at line 126)

- [ ] **Step 1: Open topics.yml and locate the end of the file**

The file ends at line 126 (`github_project: null` under `rlhf-agentic`). You will append after this line.

- [ ] **Step 2: Append the new topic entry**

Add the following block at the end of `topics.yml` (after the last `github_project: null`):
...
```yaml
  ...
  - id: llm-alignment-feedback-infra
    ...
    arxiv_feeds:
      - { name: "cs.CL", url: "https://rss.arxiv.org/rss/cs.CL" }
...
t = find_topic('llm-alignment-feedback-infra')
print('id:', t['id'])
print('name:', t['name'])
...
If you see a YAML parse error, check for inconsistent indentation — all topic entries use 2-space indentation.

- [ ] **Step 4: Commit**

```bash
git add topics.yml
git commit -m "feat: add llm-alignment-feedback-infra topic to topics.yml"
```
...
**Files:**
- Create: `topics/llm-alignment-feedback-infra/README.md`

- [ ] **Step 1: Create the topic directory**

```bash
mkdir topics/llm-alignment-feedback-infra
```

- [ ] **Step 2: Create the README**

Create `topics/llm-alignment-feedback-infra/README.md` with the following content:
...
Run:
```bash
ls topics/llm-alignment-feedback-infra/
```
...
```bash
git add topics/llm-alignment-feedback-infra/README.md
git commit -m "feat: add llm-alignment-feedback-infra topic README with initial resources"
```
...
- [ ] **Step 1: Confirm load_topics returns all 6 topics**
...
import os
d = topic_dir('llm-alignment-feedback-infra')
r = topic_readme('llm-alignment-feedback-infra')
...
If no changes needed, no commit required. If you had to fix anything in the previous steps:

```bash
git add -p
git commit -m "fix: correct topic config or readme for llm-alignment-feedback-infra"
```
