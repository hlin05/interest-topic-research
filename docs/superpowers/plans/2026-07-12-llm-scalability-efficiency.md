# LLM Scalability & Efficiency Topic Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add `llm-scalability-efficiency` as a new weekly-researched topic covering the full efficiency stack for LLMs — from parameter-efficient fine-tuning and compute-efficient architectures through quantization, distributed training, and high-throughput serving infrastructure.

**Architecture:** Two files need to be created or modified. `topics.yml` gets a new topic entry read dynamically by `scripts/topic_config.py` — no workflow changes required. `topics/llm-scalability-efficiency/README.md` is the human-facing resource list, seeded with landmark projects and papers across five sub-areas.

**Tech Stack:** YAML (topic config), Markdown (README), Python (validation only)

---

### Task 1: Add topic entry to topics.yml

**Files:**
- Modify: `topics.yml` (append after the last entry)

- [ ] **Step 1: Confirm the current last entry in topics.yml**

Run:
```bash
python -c "
from scripts.topic_config import load_topics
topics = load_topics()
print('Current last topic:', topics[-1]['id'])
...
Expected output:
```
Current last topic: llm-alignment-feedback-infra
Total topics: 7
```

- [ ] **Step 2: Append the new topic entry**

Add the following block at the very end of `topics.yml` (after the final `github_project: null` line). Use 2-space indentation to match existing entries:
...
```yaml
  ...
  - id: llm-scalability-efficiency
    ...
    arxiv_feeds:
      ...
      - { name: "cs.AR", url: "https://rss.arxiv.org/rss/cs.AR" }
      - { name: "cs.DC", url: "https://rss.arxiv.org/rss/cs.DC" }
    inclusion_criteria:
      min_match: 3
      checklist:
...
- [ ] **Step 3: Validate the YAML parses correctly and topic is loadable**

Run:
```bash
python -c "
from scripts.topic_config import find_topic
t = find_topic('llm-scalability-efficiency')
print('id:', t['id'])
print('name:', t['name'])
print('checklist items:', len(t['inclusion_criteria']['checklist']))
print('arxiv feeds:', len(t['arxiv_feeds']))
print('exclude items:', len(t['exclude']))
"
```
...
If you see a YAML parse error, check for inconsistent indentation — all topic entries use 2-space indentation throughout.

- [ ] **Step 4: Commit**

```bash
git add topics.yml
git commit -m "feat: add llm-scalability-efficiency topic to topics.yml"
```

---

### Task 2: Create the topic README

**Files:**
- Create: `topics/llm-scalability-efficiency/README.md`

- [ ] **Step 1: Create the topic directory**

```bash
mkdir topics/llm-scalability-efficiency
```

- [ ] **Step 2: Create the README**

Create `topics/llm-scalability-efficiency/README.md` with the following content exactly:
...
- [ ] **Step 3: Verify the topic directory and file exist**

Run:
```bash
ls topics/llm-scalability-efficiency/
```
...
```bash
git add topics/llm-scalability-efficiency/README.md
git commit -m "feat: add llm-scalability-efficiency topic README with initial resources"
```

---

### Task 3: Verify the full pipeline sees the new topic

- [ ] **Step 1: Confirm load_topics returns all 8 topics**
...
Total topics: 8
 ...
 - llm-alignment-feedback-infra
 - llm-scalability-efficiency
```

- [ ] **Step 2: Confirm topic_dir and topic_readme paths resolve correctly**

Run:
```bash
python -c "
from scripts.topic_config import topic_dir, topic_readme
import os
d = topic_dir('llm-scalability-efficiency')
r = topic_readme('llm-scalability-efficiency')
print('dir exists:', os.path.isdir(d))
print('readme exists:', os.path.isfile(r))
"
```
...
If no changes were needed, no commit required. If you had to fix anything:

```bash
git add -p
git commit -m "fix: correct topic config or readme for llm-scalability-efficiency"
```
